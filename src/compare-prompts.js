...
קיבלת רשימת כותרות ותקצירי RSS כלכליים מ-24 השעות האחרונות.
המידע חלקי, ולכן עליך להיות שמרן, מדויק וזהיר.

משימה:
בחר 5 עד 7 אירועים כלכליים חשובים בלבד מתוך הכתבות שסופקו.

כללי אמינות מחייבים:
1. אסור להוסיף עובדות, מדינות, חברות, סכומים, מלחמות, רגולטורים, החלטות ריבית או אירועים שלא הופיעו במפורש בכותרות או בתקצירים.
2. אסור לחבר בין שתי כתבות שונות לאותו אירוע אלא אם מדובר בבירור באותו נושא.
3. אם יש ספק — אל תחבר, ואל תסיק.
4. אם המידע חלקי, כתוב במפורש: "על בסיס המידע הזמין".
5. אל תכתוב שהייתה השפעה בפועל על שווקים, מטבעות או ישראל אם זה לא מופיע במידע שסופק. במקום זאת כתוב "עשוי להשפיע".
6. תן עדיפות לנושאים בעלי קשר ברור לשווקים, ריבית, אינפלציה, מט"ח, אנרגיה, טכנולוגיה, רגולציה, סחר גלובלי או ישראל.
7. אל תחזור על אותו אירוע פעמיים.
8. כתוב בעברית מקצועית, קצרה וברורה.
9. החזר JSON תקין בלבד, ללא Markdown וללא טקסט נוסף.

פורמט חובה:
[
  {
    "title": "כותרת קצרה בעברית, נאמנה למידע שסופק",
    "eventSummary": "מה קרה ב-2-3 משפטים, ללא תוספות שלא הופיעו במקור",
    "globalImpact": "משמעות גלובלית אפשרית בלבד, בזהירות",
    "israelImpact": "משמעות אפשרית לישראל בלבד, בזהירות",
    "importance": "גבוהה / בינונית / נמוכה",
    "category": "שווקים / ריבית / אינפלציה / אנרגיה / טכנולוגיה / ישראל / רגולציה / אחר",
    "sourceIds": [1,2]
  }
...
<head>
  ...
  <title>השוואת פרומפטים - ${escapeHTML(pageLabel)}</title>
  <style>
    :root {
      --navy: #0f172a;
      --blue: #2563eb;
...
<body>
  <header class="top-hero">
    <h1>הבוקר הכלכלי</h1>
    <p>עמוד בדיקה חד־פעמי להשוואת איכות פרומפטים</p>
    <span class="test-label">${escapeHTML(pageLabel)}</span>
  </header>

  <div class="system-bar">
    <div class="system-inner">
      <div>עודכן לאחרונה: <strong>${escapeHTML(now)}</strong></div>
      <div>מצב מערכת: <span class="ok">● בדיקה הופקה בהצלחה</span></div>
    </div>
  </div>
  ...
  <main>
    ...
    <p class="disclaimer">
      זהו עמוד בדיקה בלבד. הסיכום מבוסס על כותרות ותקצירי RSS, ולכן אינו מחליף קריאה מלאה של הכתבות המקוריות.
    </p>
  </main>
</body>
</html>`;
...
async function main() {
  ...
  fs.writeFileSync(
    "docs/prompt-original.html",
    generateHTML(originalDigest, articles, feedStatus, "פרומפט מקורי"),
    "utf8"
  );

  fs.writeFileSync(
    "docs/prompt-strict.html",
    generateHTML(strictDigest, articles, feedStatus, "פרומפט מחמיר / שמרני"),
    "utf8"
  );

  console.log("Comparison pages generated successfully");
}
